                ...
                if currency in self._data_visible:
                    ...
                    if currency not in self._data_lines:
                        ...
                        self._data_lines[currency]['high'] = self.ax.plot(
                            # Unpack a list of tuples into two lists, passed as individual args.
                            x, high,
                            pen=pg.mkPen(self.get_currency_color(
                                currency), width=2, style=Qt.DotLine)
                        )
                        ...
                        self._data_lines[currency]['close'] = self.ax.plot(
                            ...
                            x, close,
                            pen=pg.mkPen(
                                self.get_currency_color(currency), width=3)
                        )
                    else:
